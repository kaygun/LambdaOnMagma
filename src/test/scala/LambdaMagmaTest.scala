...
class LambdaMagmaTestSuite extends AnyFlatSpec with Matchers with ScalaCheckPropertyChecks {
  ...
  // Expression Types
  "Expr smart constructors" should "create correct structures" in {
    Expr.v("x") shouldBe Leaf(Left(Var("x")))
    Expr.l("x")(Expr.v("x")) shouldBe Leaf(Right(Lambda(Var("x"), Leaf(Left(Var("x"))))))
  ...
  "Expr.freeVars" should "correctly identify free variables in De Bruijn terms" in {
    // λx.x (no free vars)
    val identity = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0)))))): BinTree[Expr[Int]]
    Expr.freeVars(identity) shouldBe Set.empty
    
    // λx.y (free var y with index 1)
    val lambdaWithFree = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(1)))))): BinTree[Expr[Int]]
    Expr.freeVars(lambdaWithFree) shouldBe Set(1)
    
    // x y (two free vars)
    val application = Branch(Leaf(Left(Var(0))), Leaf(Left(Var(1)))): BinTree[Expr[Int]]
    Expr.freeVars(application) shouldBe Set(0, 1)
  }

  "Expr.substitute" should "perform correct De Bruijn substitution" in {
    val target = Leaf(Left(Var(0))): BinTree[Expr[Int]]
    val replacement = Leaf(Left(Var(5))): BinTree[Expr[Int]]
    Expr.substitute(target, 0, replacement) shouldBe replacement
    
    // Substitute in lambda body: λ.1 with 0 -> Var(5) becomes λ.6 (shifted)
    val lambda = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(1)))))): BinTree[Expr[Int]]
    val substituted = Expr.substitute(lambda, 0, replacement)
    substituted shouldBe Leaf(Right(Lambda(Var(0), Leaf(Left(Var(6))))))
  }

  "Expr.betaStep" should "perform single beta reduction step" in {
    // (λx.x) y -> y
    val identity = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0)))))): BinTree[Expr[Int]]
    val arg = Leaf(Left(Var(5))): BinTree[Expr[Int]]
    val application = Branch(identity, arg): BinTree[Expr[Int]]
    
    Expr.betaStep(application) shouldBe arg
  }

  it should "find leftmost-outermost redex" in {
    // (λx.x) ((λy.y) z) should reduce the outer application first
    val identity = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0)))))): BinTree[Expr[Int]]
    val innerApp = Branch(identity, Leaf(Left(Var(2)))): BinTree[Expr[Int]]
    val outerApp = Branch(identity, innerApp): BinTree[Expr[Int]]
    
    Expr.betaStep(outerApp) shouldBe innerApp
  }

  "Expr.betaReduce" should "reduce to normal form" in {
    // (λx.λy.x) a b -> a
    val true_expr = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Left(Var(1))))))))): BinTree[Expr[Int]]
    val a = Leaf(Left(Var(10))): BinTree[Expr[Int]]
    val b = Leaf(Left(Var(11))): BinTree[Expr[Int]]
    val application = Branch(Branch(true_expr, a), b): BinTree[Expr[Int]]
    
    Expr.betaReduce(application) shouldBe a
  }

  "Expr.eta" should "perform eta reduction" in {
    // λx.(f x) -> f (when x not free in f)
    val f = Leaf(Left(Var(1))): BinTree[Expr[Int]]
    val etaExpandedForm = Leaf(Right(Lambda(Var(0), Branch(f, Leaf(Left(Var(0))))))): BinTree[Expr[Int]]
    
    Expr.eta(etaExpandedForm) shouldBe Leaf(Left(Var(0)))
  }

  "Expr.toDeBruijn" should "convert named expressions to De Bruijn" in {
    val namedIdentity = Leaf(Right(Lambda(Var("x"), Leaf(Left(Var("x")))))): BinTree[Expr[String]]
    Expr.toDeBruijn(namedIdentity) shouldBe Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0))))))
  }

  it should "handle free variables correctly" in {
    // Let's test simpler cases first
    // Simple free variable case: just "y"
    val freeVar = Leaf(Left(Var("y"))): BinTree[Expr[String]]
    val freeVarResult = Expr.toDeBruijn(freeVar, List.empty, Set("y"))
    freeVarResult shouldBe Leaf(Left(Var(0)))
    
    // Lambda with free variable: λx.y
    val namedExpr = Leaf(Right(Lambda(Var("x"), Leaf(Left(Var("y")))))): BinTree[Expr[String]]
    val result = Expr.toDeBruijn(namedExpr, List.empty, Set("y"))
  ...
  "Church numerals" should "be constructible and work correctly" in {
    // Church zero: λf.λx.x
    val zero = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0))))))))): BinTree[Expr[Int]]
    Expr.freeVars(zero) shouldBe Set.empty
    
    // Church one: λf.λx.f x
    val one = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Branch(Leaf(Left(Var(1))), Leaf(Left(Var(0)))))))))): BinTree[Expr[Int]]
    Expr.freeVars(one) shouldBe Set.empty
    
    // Successor function: λn.λf.λx.f (n f x)
    val succ = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), 
      Branch(Leaf(Left(Var(1))), Branch(Branch(Leaf(Left(Var(2))), Leaf(Left(Var(1)))), Leaf(Left(Var(0)))))))))))))): BinTree[Expr[Int]]
    
    // Test: succ zero should be equivalent to one
    val succZero = Branch(succ, zero): BinTree[Expr[Int]]
    val reduced = Expr.betaReduce(succZero)
    
    // The result should be alpha-equivalent to one (both are closed terms)
    Expr.freeVars(reduced) shouldBe Set.empty
  }

  "Church booleans" should "work correctly with conditional logic" in {
    // Church true: λx.λy.x
    val churchTrue = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Left(Var(1))))))))): BinTree[Expr[Int]]
    
    // Church false: λx.λy.y
    val churchFalse = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0))))))))): BinTree[Expr[Int]]
    
    // Test conditional: true a b -> a
    val a = Leaf(Left(Var(10))): BinTree[Expr[Int]]
    val b = Leaf(Left(Var(11))): BinTree[Expr[Int]]
    val trueCondition = Branch(Branch(churchTrue, a), b): BinTree[Expr[Int]]
    
    Expr.betaReduce(trueCondition) shouldBe a
    
    // Test conditional: false a b -> b
    val falseCondition = Branch(Branch(churchFalse, a), b): BinTree[Expr[Int]]
    Expr.betaReduce(falseCondition) shouldBe b
  }

  "Combinators" should "work correctly" in {
    // I combinator: λx.x
    val I = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0)))))): BinTree[Expr[Int]]
    
    // K combinator: λx.λy.x
    val K = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Left(Var(1))))))))): BinTree[Expr[Int]]
    
    // S combinator: λx.λy.λz.x z (y z)
    val S = Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), Leaf(Right(Lambda(Var(0), 
      Branch(Branch(Leaf(Left(Var(2))), Leaf(Left(Var(0)))), Branch(Leaf(Left(Var(1))), Leaf(Left(Var(0)))))))))))))): BinTree[Expr[Int]]
    
    // Test I: I a -> a
    val a = Leaf(Left(Var(5))): BinTree[Expr[Int]]
    val Ia = Branch(I, a): BinTree[Expr[Int]]
    Expr.betaReduce(Ia) shouldBe a
    
    // Test K: K a b -> a
    val b = Leaf(Left(Var(6))): BinTree[Expr[Int]]
    val Kab = Branch(Branch(K, a), b): BinTree[Expr[Int]]
    Expr.betaReduce(Kab) shouldBe a
    
    // Test SKK = I (famous combinator identity)
    val SKK = Branch(Branch(S, K), K): BinTree[Expr[Int]]
    val SKKa = Branch(SKK, a): BinTree[Expr[Int]]
    Expr.betaReduce(SKKa) shouldBe a
  }
  ...
  "Beta reduction" should "be confluent for simple cases" in {
    ...
    val identity = Leaf(Right(Lambda(Var(0), Leaf(Left(Var(0)))))): BinTree[Expr[Int]]
    val arg = Leaf(Left(Var(1))): BinTree[Expr[Int]]
    val app = Branch(identity, arg): BinTree[Expr[Int]]
    
    val step1 = Expr.betaStep(app)
    val step2 = Expr.betaStep(step1)
  ...
  "Alpha equivalence" should "work via De Bruijn representation" in {
    ...
    val db1 = Expr.toDeBruijn(expr1)
    val db2 = Expr.toDeBruijn(expr2)
  ...
  "Complex reductions" should "work correctly" in {
    ...
    val step1 = Branch(churchTrue, a): BinTree[Expr[Int]]
    val step2 = Branch(step1, b): BinTree[Expr[Int]]
    
    val reduced = Expr.betaReduce(step2)
    reduced shouldBe a
  }
}
